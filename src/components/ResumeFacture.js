...
const ResumeFacture = () => {
  ...
  const handleEnrDevis = () => {
    setLoader(true);
    axios.post("/api/administrateur/enregistrer-devis", { immatriculationDepot, observationsDepot, facturation, clientFacturation, totalComplet, signatureOptimom })
      .then(response => {
        setLoader(false);
  ...
  const handleValDevis = () => {
    setLoader(true);
    axios.post("/api/administrateur/valider-devis", { immatriculationDepot, observationsDepot, facturation, clientFacturation, totalComplet, signatureOptimom, signatureExutoire })
      .then(response => {
        setLoader(false);
        if (response.data.success) {
          setClientFacturation(null);
    ...
    <>
      ...
      >
        <Toast bg="success" onClose={() => setShowToast(false)} show={showToast} delay={3000} autohide>
          ...
          <Toast.Body style={{ color: "white" }}>Nouveau dépots enregistré avec succès.</Toast.Body>
        </Toast>
      </ToastContainer>
      <Card bg="dark" text="white" border="light">
        ...
        </Card.Header>
        {
          ...
          <Card.Body>
            ...
            <Divider />
            <div className={styles.signatureTitle}>
              <h5 className={styles.titleResume}>Signature EXUTOIRE </h5>
              <Button variant="warning" onClick={() => { signatureExutoireRef.current.clear(); setSignatureExutoire(null) }} >Effacer la signature</Button>
            </div>
