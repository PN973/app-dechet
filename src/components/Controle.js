...
const Controle = ({ onHide }) => {
  ...
  const { facturation, setFacturation, setClientFacturation, clientFacturation, updateTotalControle, controleAcces, setControleAcces } = useMyContext();
  ...
  const handleChangeQuantite = (id, e) => {
    ...
    updateTotalControle(id, quantite);
  };

  const handleControleSend = (event) => {

    //console.log(agentSelect, clientFacturation, dateSelected, heureSelected, immatSelected, communeSelected, controleAcces)
    ...
    if (form.checkValidity() === false) {
      ...
      axios.post("/api/administrateur/enregistrer-controle", { communeEvacuation, tags, agent: agentSelect, client: clientFacturation, dateSelected, heureSelected, immatSelected, communeSelected, produits: controleAcces, isEntreprise })
        .then(response => {
          setLoadingBtn(false);
  ...
  const handleSelectedAgent = (e) => {
    ...
    const agent = listeAgent.find(elt => elt.id === e.value);
    const filteredClient = listeClients.filter(elt => elt.decheterieID === agent.decheterieID);
    ...
    const filteredMatieres = listeMatieres.filter(elt => elt.decheterieID === agent.decheterieID);
      ...
      <Card>
        ...
        <Card.Body>
          <Form ref={formRef} noValidate validated={formValidated}>
            ...
            <Form.Group>
              <Form.Label>Sélectionner la déchèterie*</Form.Label>
              <Select
            ...
            <Form.Group>
              ...
              <Form.Control required defaultValue={dateSelected} type="date" min={dateMinEvacuation} onInput={(e) => setDateSelected(e.target.value)} />
            </Form.Group>
                      ...
                      controleAcces.map((res, index) => {
                        ...
                        return (
                          <tr key={res.id}>
                            ...
                            <td>
                              <Form.Control onChange={e => handleChangeQuantite(res.produit.id, e)} type="number" required placeholder="Quantité" defaultValue={res.quantite} />
                            </td>
                          </tr>
                        )
                      })
