...
const Evacuations = () => {
  ...
  const { matieresEvacuees, setMatieresEvacuees } = useMyContext();
  ...
  const handleEnvoyerValidationEvacuation = async (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      axios.post("/api/administrateur/envoi-evacuations", { imageRecapitulatif, typeEvacuation, decheterie, dateEvacuation, heureEvacuation, immatriculation, nomChauffeur, numBenne, matieresEvacuees, peseeExutoire, typePesee, peseeEnEntree, peseeEnSortie, peseeNet, observations, listeImagesObservations, signatureOptimom })
        .then(response => {
          setLoadingBtn(false);
  ...
  const handleValiderEvacuation = async (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      axios.post("/api/administrateur/valider-evacuations", { imageRecapitulatif, typeEvacuation, decheterie, dateEvacuation, heureEvacuation, immatriculation, nomChauffeur, numBenne, matieresEvacuees, peseeExutoire, typePesee, peseeEnEntree, peseeEnSortie, peseeNet, observations, listeImagesObservations, signatureOptimom, signatureExutoire })
        .then(response => {
          setLoadingBtn(false);
          setFormValidated(false);
          if (response.data.success) {
          ...
          if (row.etat === "attente") {
            return (
              <div className="optionsButton">
                ...
                <Button variant="warning" onClick={() => router.push(`/dashboard/evacuations/details/${row.id}`)}>Détails</Button>
            ...
            return (
              <div className="optionsButton">
                <Button variant="primary" onClick={() => router.push(`/dashboard/evacuations/details/${row.id}`)}>Imprimer</Button>
              </div>
  ...
  const handleSelectedDecheterie = (e) => {
    ...
    const allMatieres = listeMatieres.filter(elt => elt.decheterieID === e.target.value);
    if (allMatieres.length > 0) {
      setSelectedMatieres(allMatieres);
    };
            ...
            <Modal.Body>
              <Form ref={formRef} noValidate validated={formValidated} >
                ...
                <div style={{ margin: "20px 0" }}>
                  {
                    listeMatieres.length > 0 && decheterie.length > 0 && selectedMatieres.length > 0 ?

                      selectedMatieres.map((res, index) => {
                        return (
                          <Form.Check
                            key={res.id}
                            type="checkbox"
                ...
                */}
                ...
                <Form.Group className="mb-3" controlId="">
                  <Form.Label>Photo du ticket</Form.Label>
                  {
                ...
                <Card style={{ marginTop: 20 }} >
                  <Card.Header>Signature CHAUFFEUR</Card.Header>
                  <Card.Body>
                    <SignatureCanvas
                  ...
                  <Card.Footer>
                    <Button variant="danger" onClick={() => { signatureExutoireRef.current.clear(); setSignatureExutoire(null) }}>Effacer</Button>
                  </Card.Footer>
                </Card>
...
export default ProtectedRoute(Evacuations);