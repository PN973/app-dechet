...
const ControleAcces = () => {
    ...
    axios.get("/api/administrateur/enregistrer-controle")
      .then(response => {
        if (response.data.success) {
          const sortedControles = response.data.controles.sort((a, b) => b.numero - a.numero);
  ...
  const columns = [
    ...
    {
      name: <p style={{ fontWeight: "bold", fontSize: 16 }}>Agents</p>,
      selector: (row, index) => row.agent.label,  // Génère un ID en fonction de l'index
      sortable: true,
    },
    {
  ...
  return (
    <>
      <NavBarComp title="Contrôle d'accès" />
            ...
            loadingData ?
              ...
              <div ref={tableRef}>
                <DataTable
                  ...
                  title={"Liste des contrôles d'accès"}
...
export default ProtectedRoute(ControleAcces);