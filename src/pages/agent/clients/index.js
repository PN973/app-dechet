...
const Clients = () => {
      ...
      axios.get("/api/administrateur/clients")
        .then(response => {
          if (response.data.success) {
            const filteredData = response.data.clients.filter(item => item.agentID === data.id);
            setListeClients(filteredData);
  ...
  const handleSubmitClient = (event) => {
    ...
    if (form.checkValidity() === false) {
      ...
      const userData = getCookie("agent");
      const data = JSON.parse(userData);
      axios.post("/api/agent/clients", { tags, id: data.id, decheterie, isEntreprise, nomSociete, numSiret, nomGerant, prenomGerant, immatriculation, courriel, telephone, ville, codePostal, adresse, nomParticulier, prenomParticulier })
        .then(response => {
          setLoadingBtn(false);
...
export default ProtectedRouteAgent(Clients);