...
const handler = async (req, res) => {
 if (req.method === "POST") {
  ...
  const { imageRecapitulatif, typeEvacuation, decheterie, dateEvacuation, heureEvacuation, immatriculation, nomChauffeur, numBenne, matieresEvacuees, peseeExutoire, typePesee, peseeEnEntree, peseeEnSortie, peseeNet, observations, listeImagesObservations, signatureOptimom, signatureExutoire } = req.body;


  if (typeEvacuation) {

   if (decheterie !== "" && dateEvacuation !== "" && heureEvacuation !== "" && immatriculation !== "" && nomChauffeur !== "" && numBenne !== "" && matieresEvacuees.length > 0 && /* peseeExutoire !== "" && typePesee !== "" &&*/ peseeEnEntree !== "" && peseeEnSortie !== "" && peseeNet !== "" && signatureOptimom !== "" && signatureExutoire !== "") {
    ...
    await set(ref(db, `${structures.evacuations}/${evacuationID}`), {
     ...
     signatureExutoire: signatureExutoire,
    ...
    res.status(200).json({
     success: true, evacuation: updatedRecord
    });
   } else {
   ...
   if (decheterie !== "" && dateEvacuation !== "" && heureEvacuation !== "" && matieresEvacuees.length > 0 /*&& immatriculation !== "" && nomChauffeur !== "" && numBenne !== "" && matieresEvacuees.length > 0 && peseeExutoire !== "" && typePesee !== "" && peseeEnEntree !== "" && peseeEnSortie !== "" && peseeNet !== "" &&*/ && signatureOptimom !== "" && signatureExutoire !== "") {
