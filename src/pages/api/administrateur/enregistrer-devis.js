...
const handler = async (req, res) => {
 ...
 if (req.method === "POST") {
  ...
  const { immatriculationDepot, observationsDepot, facturation, clientFacturation, totalComplet, signatureOptimom } = req.body;

  if (facturation.length > 0 && clientFacturation  && signatureOptimom && immatriculationDepot.length > 3) {
