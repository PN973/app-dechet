...
const handler = async (req, res) => {
 ...
 if (req.method === "POST") {
  ...
  const { immatriculationDepot, observationsDepot, facturation, clientFacturation, totalComplet, signatureOptimom, signatureExutoire } = req.body;

  if (facturation.length > 0 && clientFacturation && signatureOptimom && signatureExutoire && immatriculationDepot.length > 3) {
